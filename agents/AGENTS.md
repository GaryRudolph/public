...
- **Plan around model-tier stop points** — tag every executable step `[deep]` (architecture, ambiguous problems, hard debugging, security-sensitive review), `[exec]` (cross-file refactors, repo-aware implementation, judgement-heavy tests), or `[fast]` (mechanical, fully-spec'd, single-concern work — only when the downgrade checklist passes). Tag only the executable (deepest) heading level; leave milestone/phase/section headings untagged. Place the tag immediately after the title separator: `#### s1 - [deep] Title` or `#### 3. [exec] Add tests` — do not rename or renumber. Insert a STOP at every `[deep]` boundary; insert a STOP at `[exec]` ↔ `[fast]` boundaries only when the `[fast]` run is ≥ 3 steps, otherwise promote those steps to `[exec]` to avoid model-swap thrash. Each STOP emits the next model for both Cursor and Claude Code plus a copy-pasteable handoff prompt and a "stop at next STOP marker" limit. Models per tier: deep → Opus 4.8 xhigh (Cursor `claude-opus-4-8-thinking-xhigh` / Claude Code `/model opus` + extended thinking), exec → Sonnet 4.6 medium (Cursor `claude-4.6-sonnet-medium-thinking` / Claude Code `/model sonnet`), fast → Composer 2.5 fast (Cursor `composer-2.5-fast`) / Haiku 4.5 (Claude Code `/model haiku`), no extended thinking. If your harness supports skills (Cursor, Claude Code), apply the `personal-plan-model-tiers` skill. See `~/Projects/personal/public/standards/documentation.md` "Model-tier stop points" for the full reference (tiers, downgrade checklist, no-thrash rule, model picker, STOP-marker template)
