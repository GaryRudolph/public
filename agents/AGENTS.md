...
- **Plan around model-tier stop points** — tag every executable step `[deep]` (architecture, ambiguous problems, hard debugging, security-sensitive review), `[exec]` (cross-file refactors, repo-aware implementation, judgement-heavy tests), or `[fast]` (mechanical, fully-spec'd, single-concern work — only when the downgrade checklist passes). Tag only the executable (deepest) heading level; leave milestone/phase/section headings untagged. Place the tag immediately after the title separator: `#### s1 - [deep] Title` or `#### 3. [exec] Add tests` — do not rename or renumber. Tagging records honest complexity and is never rewritten for thrash reasons. Insert a STOP at every `[deep]` boundary; insert a STOP at `[exec]` ↔ `[fast]` boundaries only when the `[fast]` run is ≥ 3 steps, otherwise fold those steps into the adjacent `[exec]` wave so they run on the `[exec]` model — leaving the `[fast]` tags in place so the plan still shows how complex the work actually was (the fold changes the wave's execution tier, not the tag). Each STOP emits the next model for both Cursor and Claude Code plus a copy-pasteable handoff prompt and a "stop at next STOP marker" limit. Models per tier: deep → Opus 4.8 xhigh (Cursor `claude-opus-4-8-thinking-xhigh` / Claude Code `/model opus` + extended thinking), exec → Sonnet 4.6 medium (Cursor `claude-4.6-sonnet-medium-thinking` / Claude Code `/model sonnet`), fast → Composer 2.5 standard (Cursor `composer-2.5` standard) / Haiku 4.5 (Claude Code `/model haiku`), no extended thinking. If your harness supports skills (Cursor, Claude Code), tag first with `personal-plan-tag-tiers` when you just want to see a plan's complexity, then apply `personal-plan-model-tiers` (STOP-and-swap) to execute; on Cursor, use `personal-plan-orchestrate` instead when you want the cascade run automatically. Both drivers auto-run `personal-plan-tag-tiers` if the plan isn't tagged yet, so invoke either directly when you already know how you want to run it. As each group finishes, keep both surfaces in sync: append ` (done)` to its headings, flip its native todo to `completed`, and update the `Status:` line in the Kickoff block; when the last group finishes, flip the Kickoff marker to `--- KICKOFF: plan complete ---` and append a Completion summary at the bottom of the plan file. See `~/Projects/personal/public/standards/plan-execution.md` "Model-tier stop points" for the full reference (tiers, downgrade checklist, no-thrash rule, model picker, STOP-marker template, progress tracking)
