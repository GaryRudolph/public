...
- **Plan around model-tier stop points** — when producing a plan, tag every milestone/step as `[deep]` (architecture/design decisions, ambiguous problems, hard debugging, security-sensitive review) or `[exec]` (well-specified refactors, mechanical edits, applying a known design, test scaffolding, doc updates). Stop at every boundary between `[deep]` and `[exec]` blocks so I can swap to a deeper or cheaper model, or delegate the `[exec]` block to a subagent on a less capable model when the harness supports it (Cursor `Task`, Claude Code `Task`, etc.). See `~/Projects/personal/public/standards/documentation.md` "Model-tier stop points"
