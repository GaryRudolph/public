...
| `personal-allowlist-scout`             | skill   | Scans every git repo in the open workspace, classifies build commands by access/intent, and proposes the safe subset (Inspect + Build & test only) missing from the existing per-harness allowlists; confirms repo by repo; allow-only, fail-closed; routes global-promotion candidates to agerpoint.json or personal.json based on repo origin |
| `personal-makefile`                    | skill   | Audit and align Makefiles to the personal standard (target vocabulary, self-documenting help, workspace/polyrepo delegation, danger guards, Makefile.md). Dry-run first; apply repo-by-repo with confirmation; never bootstraps a missing workspace |
